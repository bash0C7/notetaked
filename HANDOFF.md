...
- **open issue #18・#8・#4・#2をまとめて対応・branch `followup-issue-18-8-4-2`でdraft PR作成（2026-09-20）**: `/compact`後、user指定通り4件を並行して着手（`main`直下に一旦4commit積んだ後、branchへ退避してdraft PR化。origin/mainへの直push・自動merge提案はしていない）。draft PR: <!-- PR URL, 作成後に追記 -->
  ...
  - **#4（ペアリングコード方式の代替）: 「後がけmergeモード」specを作成（commit `b142c19`、`docs/superpowers/specs/2026-09-20-merge-mode-design.md`）。plan作成・実装は未着手（specのuserレビュー待ち）**。user決定（2026-09-20）: merge処理自体は本人かどうか問わない汎用設計、ただし自動検出ルートはiCloud経由・本人所有機材限定から着手。起動は自動検出が望ましいとのuser回答を受け、時刻合致判定は「ゆるい重なり判定＋既存Reconciler（±1秒・Dice 0.5）に実質的なフィルタを委ねる」設計とした（Claude判断、根拠はspec内に記載）。既存の`docs/superpowers/specs/2026-09-16-icloud-pairing-design.md`（今すぐ繋ぐ・iCloud鍵配布案）とは併存する別選択肢という位置付け
  - **#2（iPhone方位軸校正、低優先度）: 文書調査のみ・コード修正は保留（commit `25531d4`）**。user指定（2026-09-20）: user操作（実機を動かして4方向から発話）を使わず判別してほしいとの要望を受け、Apple公式文書（WWDC25 session 251等）を調査。X/Y/Zの**数学的役割**（前後・左右・上下）は確認できたが、**iPhone実機の物理筐体でどちらが+Xかは公式文書に記載が無く**、実機データからの逆算での当てずっぽう修正は避けて保留。`DirectionEstimatorTests`に180°/270°の既知方位合成テストを追加し、既存の数式ロジック自体は0/90/180/270いずれも正しく復元することを確認（数式は正しい、軸の物理対応のみ未確認）。specの検証手順を改訂し、次回実機時に上端/右/下端/左の4方向を1回測るだけで「軸入れ替えが要るか」「単純offsetで済むか」を判定できるようにした
  - **次回セッションで拾うべきこと**: (a) このdraft PRの実機検証状況を見てmerge判断（#18は実機済み、#8は実会話待ち） (b) #4のspecをuserがレビューし、plan作成へ進めるか判断 (c) #2は次回iPhone実機作業のついでに4方向計測 (d) #8は次回の自然な収録で実会話確認
