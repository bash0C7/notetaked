...
- **issue #7 / #8 / #6 / #3 / #5 実装済み・未検証、#4は検討のみ（コード変更なし）（2026-09-16、Claude Code on the web、Linux）**: このセッションはSwiftツールチェーンが無く`make verify`・実機確認ができないため、コード変更のみでMac側の検証が未実施。plan: `docs/superpowers/plans/2026-09-16-remaining-issue-fixes.md`
  ...
  - **`make verify`で1件ビルド失敗→修正（2026-09-16、Mac実機、commit `aa927ee`、このLinuxセッションの手元には未反映）**: `ServeSession.swift:185`の三項演算子がクロージャを構築する式（`source == .system ? { .system } : { InputDeviceProbe.current() }`）でswift-frontendが内部エラー（`failed to produce diagnostic for expression`）を起こしたため、if/elseで同じ`@Sendable`クロージャを組み立てる形に書き換えて回避（挙動は変えていない）。**この commit はMac側セッションでまだpushされていない**。次にMac側で作業する時はまず`git push`してこのbranchに反映すること（反映されないと他セッションのcheckoutでも同じビルド失敗が再現する）
