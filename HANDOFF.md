...
- **人の操作が要る残り**: 2-4・2-5（user判断。2-4の参考値は分離ありの遅延5〜16秒）。段階5・2-3後半は再検証済み（上記）。全段階の実機確認はuserの判断待ち
- **後回しの要望（user、2026-09-13）**: (1) ライブパネルのハンバーガーへの畳み込みと再展開の位置が微妙 (2) ペアリングコード方式をやめ、同じiCloudアカウントでのログイン等で相互識別したい (3) 「Macの収録開始 = セッション開始、iPhoneの開始 = そのセッションに乗る取り込み開始」という概念をUI・用語で整理する (4) issue #2（方位の軸校正） (5) 複数デバイスで同時に取れた発話の表示機器: 現状は本文の勝者（confidence→source優先度→長さ）のsegの場所ラベルを出す。**決定（user、2026-09-14）: 本文は現状の勝者のまま、場所ラベルは`level_dbfs`最大のseg（話者に最も近いマイク）の機器にする**
- **次: userが実機検証完了を明言したら、後回しの要望(1)〜(5)と「気づき」をissue化して優先順位を決める。未push commitがあればpush**
