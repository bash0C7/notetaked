...
- **次にやること（2026-09-20、user指定）**: open issue #18・#8・#4・#2を`/compact`後にまとめて着手する。個別に順番処理するのではなく、まとめて検討する方針（user: 「全open issue 18,8,4,2を一気に片付けたい」）
  - #18: pinデバイス切断後のframe drop（本文に原因箇所・実機再現手順・対応方針を記載済み、詳細は上記参照）
  - #8: 短い相槌の話者分裂（実際の会話での再検証待ち、`say`合成音声では分離のレース条件を再現できないため実機会話が必要）
  - #4: ペアリングコード方式の代替。既存spec `docs/superpowers/specs/2026-09-16-icloud-pairing-design.md`（iCloud/CloudKit案）に加え、**新たな検討方向をuserが提案（2026-09-20、要検討・未着手）**: 「後から収録したデータをmergeするモード」——各機材（Mac/iPhone/Watch/別Mac等）が単体で独立収録し、後でファイル（`.timed.jsonl`等）をAirDrop/iCloud Drive等で持ち寄って`notetaked merge`的なコマンドで統合する案。Bonjour発見・TLS PSK・ペアリングコードといった「今すぐ繋ぐ」ための仕組みを丸ごと回避でき、既存の`Reconciler`（segment統合）と`NDJSON.decodeAll`によるfold機構（resume機能で使っているものと同じ）を転用できる。**論点**: 時刻合わせ——今のPeerClientはping/pongのRTTでclock offsetをリアルタイム推定しているが、後がけmergeでは接続実績が無いため、(a)各機材のwall-clock（NTP同期前提、誤差は通常1秒未満）に頼るか、(b)音声波形の相互相関で合わせるかの選択になる。既存Reconcilerの照合窓（±1秒・Dice 0.5）はNTP程度のずれなら許容範囲内と見られるため、まずは(a)wall-clock前提で始めるのが現実的という初期見解（Claudeの推奨、brainstorming未実施）。ペアリングコード方式（今すぐ繋ぐ）を置き換えるというより、「後からmergeする」を別の選択肢として並走させる設計が良さそうという方向性のみ合意、詳細設計はこれから
  - #2: iPhone実機での方位軸校正（低優先度のまま）
