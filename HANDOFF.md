...
- **残るopen issueの扱い**: #8は自然な複数人収録で第二パスが実際に補正した証拠待ち。#4はQRペアリングで手入力を解消済みだが、iCloud等による自動識別の検討を継続。#2は絶対方位校正を保留し、方位を話者推定の補助特徴量として使う設計を別branchで扱う。Apple Watchは今回の開発ツールで検出されず、再検証待ち。
...
- **issue #18・#8・#4・#2をまとめて対応し、PR #19をmainへrebase merge（2026-09-25、`a972781`）**: followup branchと専用worktreeは削除済み。
  ...
  - **#8（短い相槌の話者分裂）: 区切り/停止時の第二パスを実装（commit `9e88004`）。issueはopenのまま（実会話での最終確認が要る）**。user判断（2026-09-20）: 単発の実会話テストは意味が薄いとして見送り、技術設計を優先。既存の`inheritedSpeakerID`（リアルタイム・過去方向のみ、5秒以内）はライブ表示用にそのまま残し、`Reconciler.resolveFallbackSpeakers()`（新規）を区切り/停止時に呼び、speakerIDが無いutteranceを直前・直後両方向の同一device utteranceから再解決（両方一致で採用/片方のみで採用/矛盾や無しならownerLabel維持）。既存の「停止後peer seg遅延到着→final.md再生成」パターンを呼び出し経路として再利用。ユニットテスト3件（直後からの継承・矛盾時fallback・近傍無しfallback）で技術的正しさを担保。実会話での最終確認は次回自然な収録機会に持ち越し
    - **検証準備を整備（2026-09-21）**: `.claude/skills/recordings/scripts/fallback-diff.sh [prefix|latest] [dir]`を新設。実際のfinal.md（第二パス適用済み）と`notetaked render`での再生成（第二パス無し、ライブ表示相当）をdiffし、第二パスがどの行の話者ラベルを実際に変えたかを機械的に示す（final.mdは実行後に元へ復元、既存の記録に対して動作確認済み）。次回自然な収録（会議のsystem音声録音、または複数人が同じmicに入る場での録音など、単一deviceに複数話者の短い相槌が混ざる状況）が取れたら、収録後にこのscriptを流すだけで判定できる。userへの依頼事項は「いつも通り録音するだけ」（特別な操作は不要）
  ...
  - **#2（iPhone方位軸校正、低優先度）: 文書調査＋仮説ベースの暫定修正・実機未確認（commit `25531d4`、`31107b9`）**。第一段（文書調査、commit `25531d4`）: Apple公式文書（WWDC25 session 251等）を調査、X/Y/Zの数学的役割（前後・左右・上下）は確認できたが**iPhone実機の物理筐体でどちらが+Xかは公式文書に記載が無く**、実機データからの逆算での当てずっぽう修正は避けて保留。`DirectionEstimatorTests`に180°/270°の既知方位合成テストを追加（数式ロジック自体は0/90/180/270いずれも正しく復元することを確認）。第二段（2026-09-20、user承認: 「うまくいかなければ仕方ない、平置きだけ対応して概ね認識できるかを試そう」）: 平置き時は+X（数学上の前後軸）が画面法線（鉛直方向）を向いているという実機データからの推定に基づき、`Recorder.foaChannels(from:)`がDirectionEstimatorへ渡す3ch目をch3(X)からch2(Z)へ差し替え（3軸直交のため+Xが鉛直ならY・Zが水平面を張るはずという推論）。**平置き限定・回転方向の正負は未確認という前提で導入、実機未確認**。`make verify`通過（iOSコンパイル含め警告ゼロ）。次回iPhone実機で4方向計測すれば「軸入れ替えが機能したか」「回転方向・offsetの追加調整が要るか」が1回で判定できる（spec「検証」節に手順あり）
    - **検証手順を1回の連続収録で完結するよう具体化（2026-09-21）**: iPhone実機（iPhone 16e）には既にZ ch差し替え版がinstall済み（QRペアリング検証時のbuildに含まれる、commit `31107b9`以降）。手順: (1) Macでセッション開始（Claudeが`mac-app`skillで自律実行） (2) iPhoneを机に平置き、画面上向き、上端が基準方向（例: Macの方）を向くよう置く（user操作） (3) iPhoneで「取り込み開始」（user操作、devicectlにUIタップ機能が無いため） (4) 基準方向から`say`等で発話→「位置1」等の一言を挟みながら90°ずつ時計回りに4回回転→各姿勢で発話（停止・再開は不要、連続収録でよい） (5) iPhoneで「取り込み終了」（user操作） (6) Claudeが`timed.jsonl`のiOS segから`direction.azimuth_deg`を発話順に抽出し、4姿勢で値が分かれるか（軸入れ替えが機能）／近い値に固まるか（機能せず、issue #2にその旨記録しこれ以上の当てずっぽうな調整はしない）を判定。userの物理操作は(2)(3)(4)(5)のみ、1回の連続収録で完結
    - **iPhone 13 Proでの実機確認（2026-09-25）**: MacからiPhone由来segを13件受信し、収録・ペアリング経路は確認できた。一方で全segが`input.spatial: false`、`direction`なしだったため、この機種ではFOA方位経路を実行できない。4方向の操作結果はZ ch差し替えの合否を示さず、#2はFOA対応iPhoneでの再検証待ち。コード変更はしていない。
    - **FOA対応iPhoneでの4方向計測（2026-09-25）**: iPhone由来segを9件受信し、全件で`input.spatial: true`と`direction`を確認した。方位値は16.5°〜281.6°まで変化したが、主に約270°と約80°に集まり、4つの90度間隔のグループを示せなかった。したがってZ ch差し替えは4方向校正に合格していない。追加のch入れ替え・符号反転・offset調整は設計承認なしに行わない。
  - **次回セッションで拾うべきこと**: (a) #8は次回の自然な複数人収録後に`fallback-diff.sh`で第二パスの実証を確認 (b) #2は別branchで、非校正の方位を話者推定の補助特徴量として使う設計から始める (c) 開発ツールがApple Watchを認識した時に実機起動・収録を確認
