...
- 通常のビルドは必ずリポジトリの`Makefile`経由で実行する（標準は`make verify`、macOS appの配置まで行う場合は`make install-app`）。単独の`swift build`や`xcodebuild`で通常ビルドを代替しない（実機検証の指定コマンドは除く）
...
- 決定論的な作業で使うskillの正本はリポジトリ内の`.claude/skills/`のみ。`.agents/`はgitignore済みで、作成・参照しない。
