...
issue #4はペアリングコード方式（今すぐ繋ぐ、TLS PSK+Bonjour）の代替として「同じiCloudアカウント等で自動的に相互識別したい」という要望から始まった。userとの検討（2026-09-20）で、これとは独立した第二の選択肢として「後から収録したデータをmergeするモード」が提案された。各機材（Mac / iPhone / Watch / 別Macなど）が単体で独立収録し、後でファイルを持ち寄って統合する。
...
- v1: 同一iCloudアカウントに属する本人の複数機材間のみ。他人の機材との統合は対象外（処理ロジックは排除しないが、発見・信頼の仕組みは作らない）
...
収録停止（`stop`）時、既存の`final.md`/`timed.jsonl`書き出しに加えて、`timed.jsonl`を**iCloud Drive上の共有フォルダ**（ubiquity container、`docs/superpowers/specs/2026-09-16-icloud-pairing-design.md`が前提とする仕組みを流用）へコピーする。ファイル名は機材識別子とprefixを含める（例: `<device-id>_<prefix>.timed.jsonl`）。生音声は含めない。
...
**時刻合致ルールの根拠（Claudeの技術判断、2026-09-20）**: 候補検出を厳しくする意味は薄い。実際の統合可否は次段の`Reconciler`（±1秒・Dice係数0.5以上）が担うため、候補検出はゆるく（false positiveを許容し、false negativeを避ける）設計する。無関係な収録が誤って候補になっても、Reconcilerがほぼ何も統合結果を出さず実害が無い。逆に候補検出を厳しくして本来統合すべき組を取りこぼす方が実害が大きい。

**時刻の基準**: 各機材のwall-clock（NTP同期前提、通常の誤差は1秒未満）をそのまま使う。音声波形の相互相関によるオフセット推定は既存のPeerClientのping/pong方式でしか実装されておらず、後がけmergeでは接続実績が無いため使えない。wall-clockのずれがReconcilerの±1秒許容を超える環境（NTP未同期など）は将来課題として残す。
...
2. 全utteranceを時系列にマージしつつ`Reconciler.apply(_:)`へ順次投入（既存のcross-device統合ロジックをそのまま再利用。同一device同士は統合しない既存ガードも維持）
3. 統合結果から新しい`<merged-prefix>.final.md`・`<merged-prefix>.timed.jsonl`を生成する。元の各機材ファイルは変更しない（追加のみ、非破壊）
...
- 既存の統合ロジック: `Sources/NotetakeCore/Reconcile/Reconciler.swift`（`Reconciler.apply(_:)`、±1秒・Dice 0.5の照合窓）
